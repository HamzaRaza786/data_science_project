...
top_funds = df.groupby("Krankenkasse")

# # --- Streamlit UI ---
# st.title("Zusatzbeitrag Forecasting & Churn Analysis")
# tabs = st.tabs(["Forecast Zusatzbeitrag", "Churn Prediction"])
...
selected_fund = st.selectbox("Select Krankenkasse:", sorted(df["Krankenkasse"].unique()))
fund_df = df[df["Krankenkasse"] == selected_fund].copy()
if len(fund_df) < 12:
    st.warning("Not enough data points for modeling.")
    st.stop()

# --- Forecast Zusatzbeitrag ---
